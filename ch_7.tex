\section{Глава 7. Характеристические функции. ЦПТ.}

\subsection{Характеристические функции}
\begin{definition}
	Характеристической функцией случайной величины $\xi$ называется $\varphi_\xi(t) = \E e^{it\xi}, t \in \R$. (Формально это обратное преобразование Фурье распределения случайной величины $\xi$).
\end{definition}
...
Чтобы считать интеграл Лебега от комплекснозначной функции, будем пользоваться формулой Эйлера: $\varphi_\xi(t) = \E e^{i t \xi} = \E \cos (t \xi) + i \E \sin(t \xi)$.
 ...
 \subsection{Свойства характеристических функций (8 штук)}
 ...
 \begin{property}
	...
	\begin{property}
		...
		\begin{proof}
			...
			При $h \to 0$ выполнено $e^{ih\xi} - 1 \overset{\text{п.н.}} \longrightarrow 0 $ по теореме о наследовании сходимости. $\forall h \hookrightarrow |e^{ih\xi} - 1| \leqslant |e^{ih\xi}| + 1 = 2, ~ \E 2 < +\infty$. Следовательно, по теореме Лебега о мажорируемой сходимости, $\E |e^{ih\xi} - 1| \to \E 0 = 0$. Значит, $\varphi(t)$ равномерно непрерывна.
		\end{proof}
	\end{property}
	...
	\begin{property}
		Пусть $\varphi_\xi(t)$~--- характеристическая функция случайной величины $\xi$. $\varphi_xi(t)$ принимает действительные значения $\Leftrightarrow \xi$ имеет симметричное распределение $(\xi \overset{d}{=} -\xi)$.
		\begin{proof}
	...
	\begin{theorem}[формула обращения (б/д)]~
		...
		Пусть $\varphi(t)$ характеристическая функция функции распределения $F$. 
		
		Тогда
		\begin{enumerate}
			\item $\forall a < b$ (точки непрерывности) $F$ выполнено
	...
	\begin{proof}
		...
		\noindent \(\Rightarrow \forall n ~ \exists f_\varepsilon^n(x) = \sum\limits_{k \in K} a_k\cdot e^{\frac{ik\pi x}{n}}, ~a_k \in \mathbb{R},\) \(K\)~--- конечное подмножество \(\mathbb{Z},\) такое, что \(\forall x \in [-n, n] \hookrightarrow |f_\varepsilon^n(x) - f_\varepsilon(x)| < \frac{1}{n}.\) \(f_\varepsilon^n\)~---периодическая с периодом \(2n\) на \(\mathbb{R}\). Поскольку \(|f_\varepsilon(x)| < 2\) и \(\forall x \in [-n, n] \hookrightarrow |f_\varepsilon^n(x) - f_\varepsilon(x)| < \frac{1}{n}\), то \(|f_\varepsilon^n(x)| \leqslant 2 ~ \forall x.\) По условию, \(\forall t~\int\limits_\mathbb{R} e^{itx}dF(x) = \int\limits_\mathbb{R} e^{itx}dG(x) \Rightarrow \int\limits_\mathbb{R} f_\varepsilon^n(x)dF(x) = \int\limits_\mathbb{R} f_\varepsilon^n(x)dG(x).\)
		\begin{gather*}
			...
			\leqslant \frac{2}{n} + o(1) \Rightarrow \forall \varepsilon> 0 \hookrightarrow \int_\mathbb{R} f_\varepsilon(x) dF(x) = \int_\mathbb{R} f_\varepsilon(x)dG(x).
		\end{gather*}
	...
	\begin{theorem}[критерий назависимости]~
		...
		Пусть \(\vec{\xi} = (\xi_1, \ldots, \xi_n).\) 
		
		Тогда \((\xi_1, \ldots, \xi_n)\)~--- назависимые в совокупности $\Leftrightarrow$ 
		
		\(\varphi_{\vec{\xi}}(\vec{t}) = \prod\limits_{i = 1}^n \varphi_{\xi_i}(t_i) ~\forall \vec{t} = (t_1, \ldots, t_n) \in \R^n.\)
	\end{theorem}

	\begin{proof}
		\((\Rightarrow) ~ \varphi_{\vec{\xi}}(\vec{t}) = Ee^{i(\vec{t}, \vec{\xi})} = Ee^{i\sum\limits_{k = 1}^{n}t_k\xi_k} \overset{\text{нез-сть}}{=} \prod\limits_{k = 1}^n Ee^{it_k\xi_k} = \prod\limits_{k = 1}^n \varphi_{\xi_k}(t_k).\)
		...
		\(\varphi_G(t) = \int\limits_{\R^n}e^{i(\vec{t}, \vec{x})}dG(\vec{x}) = \int\limits_\R e^{i(\vec{t}, \vec{x})}dF_1(x_1)\cdot\ldots\cdot dF_n(x_n) \overset{\text{Фубини}}{=} \prod\limits_{k = 1}^n \int\limits_\R e^{it_kx_k} dF_k(x_k) = \prod\limits_{k = 1}^n\varphi_{\xi_k}(t_k) \overset{\text{усл}}{=}\varphi_{\vec{\xi}}(\vec{t}) \Rightarrow\) характеристическая функция \(G\) и \(\vec{\xi}\) совпадают \(\Rightarrow\) по теореме единственности \(F_\xi = G \Rightarrow F_{\vec{\xi}}(\vec{x}) = \prod\limits_{k = 1}^n F_{\xi_k}(x_k) \Rightarrow \xi_1, \ldots, \xi_n\) независимы в совокупности по критерию независимости в терминах функции распределения.
	\end{proof}
	...
	\begin{proof}
		...
		\begin{gather*}%тут не получается написать нормальный символ матожидания
			...
			= E \sum\limits_{j, k = 1}^{n}e^{it_j\xi}\cdot z_j \cdot \overline{e^{it_k\xi}}\cdot \overline{z_k} = E\left|\sum\limits_{j = 1}^{n} e^{it_j\xi}z_j\right|^2 \geqslant 0
		\end{gather*}
	...
	\begin{example}
		Любая функция вида
		\begin{center}
			\includegraphics[scale=0.32]{img/img1.png}
		\end{center}
	...
	\begin{theorem}[непрерывности для х.ф.]~
		...
		1. Пусть \(\{F_n\}_{n \geqslant 1}\)~--- последовательность функций распределения на \(\R\), \({\varphi_n(t)}_{n \in \mathbb{N}}\) --- их х.ф. 
		...
		2. (б/д) Пусть \(\forall t \in \R ~ \exists \varphi(t) = \lim\limits_{n \to +\infty} \varphi_n(t), \) причём \(\varphi(t)\) непрерывна в нуле. 
	...
	\begin{enumerate}
		...
		\begin{gather*}
			...
			= E((\xi_k - m_k)^2\cdot I(|\xi_k - m_k| > \varepsilon D_n)) \leqslant (2K)^2 EI(|\xi_k - m_k| > \varepsilon D_n) =\\
	...
	\begin{theorem}[Берри-Эссена(б/д)]~
		...
		Пусть \(\{\xi_k\}_{k \geqslant 1}\)~--- независимые одинаково распределённые случайные величины, \(E|\xi_1|^3 < +\infty, ~ E\xi_1 = a, ~ D\xi_1 = \sigma^2, ~S_n = \sum\limits_{i = 1}^{n}\xi_i; ~ T_n = \dfrac{S_n - ES_n}{\sqrt{DS_n}}.\) Тогда 
		\[
